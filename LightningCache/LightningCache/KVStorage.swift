...
fileprivate final class KVStorageFile {
    ...
    init(dataPath: String, trashPath: String) {
        ...
        trashQueue = DispatchQueue(label: "com.lightningcache.disk.trash")
    }
...
fileprivate final class KVStorageDatabase {
    ...
    fileprivate func initialize() -> Bool {
        let sql = "pragma synchronous = normal; create table if not exists manifest (key text, filename text, size integer, inline_data blob, modification_time integer, last_access_time integer, primary key(key)); create index if not exists last_access_time_idx on manifest(last_access_time);"
